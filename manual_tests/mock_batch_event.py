...
if __name__ == '__main__':
    for _ in range(NUMBER_OF_BATCHES):
        ...
        awaiting_inventory_success = True
        while len(pool) > 0:
            p.poll(0)
            # this checks for completion of the inventory process
            if True not in [len(x) > payload_length for x in pool.get_pool()] and awaiting_inventory_success:
                payload = {
                    'service': 'inventory',
                    'request_id': request_id,
                    'status': 'success'
                }
                awaiting_inventory_success = False
            else:
                payload = pool.pop()
